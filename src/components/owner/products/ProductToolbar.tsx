...
import { setProductStatusAction, deleteProductAction } from '@/lib/products/actions';
...
export function ProductToolbar({
  ...
  function handlePublishToggle() {
    ...
    startPublish(async () => {
      ...
      await setProductStatusAction(productId, isPublished ? 'draft' : 'published');
      setPublishTick('ok');
  ...
  function handleDelete() {
    if (deleting) return;
    if (!confirm(`¿Eliminar "${productTitle}"? Esto también borrará sus variantes. Esta acción no se puede deshacer.`)) return;
    startDelete(async () => {
      await deleteProductAction(productId);
      // La action ya hace redirect a /products
    });
  }
    ...
    <div className="sticky top-0 z-40 -mx-6 px-6 py-2.5 mb-4 border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 min-w-0 flex-wrap">
          <a href="/products" className="text-white/50 hover:text-white text-sm">← Productos</a>
          <span className="text-white/30">/</span>
          <span className="text-sm font-semibold truncate max-w-[280px]" title={productTitle}>{productTitle}</span>
