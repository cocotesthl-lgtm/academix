...
import { setCourseStatusAction, deleteCourseAction } from '@/lib/courses/actions';

/**
 * Toolbar sticky del editor de publicación (misma estética que
 * SiteBuilderToolbar).
 *
 * Diferencias con el del site builder:
 *   - La publicación ya tiene un status nativo ('draft' / 'published')
 *     en la row. No hay concepto de "borrador separado". Por eso el
 *     botón "Publicar" cambia el status directamente en la DB.
 *   - Incluye el botón "Eliminar" que sacamos del header viejo.
 *
 * El botón "Guardar" dispara el evento global `cp:save-all`, y el
 * <CourseEditor> escucha ese evento para llamar `form.requestSubmit()`.
 */
export function CourseBuilderToolbar({
  courseId,
  courseTitle,
  courseStatus,
  publicUrl
  ...
  courseId: string;
  courseTitle: string;
  courseStatus: 'draft' | 'published' | string;
  publicUrl: string;
}) {
  ...
  useEffect(() => {
    function onSaving() { setStatus('saving'); }
    function onSaved() {
      setStatus('saved');
  ...
  function handlePublishToggle() {
    ...
    startPublish(async () => {
      ...
      await setCourseStatusAction(fd);
      setPublishTick('ok');
  ...
  function handleDelete() {
    ...
    startDelete(async () => {
      ...
      // deleteCourseAction hace redirect, pero por si acaso:
      router.push('/courses');
    });
  }
    ...
    <div className="sticky top-0 z-40 -mx-6 px-6 py-2.5 mb-4 border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          <a href="/courses" className="text-white/50 hover:text-white text-sm">← Publicaciones</a>
          <span className="text-white/30">/</span>
          <span className="text-sm font-semibold truncate max-w-[280px]" title={courseTitle}>{courseTitle}</span>
