...
import { requireInstructor, requireOwner } from '@/lib/auth/guards';
...
/* ─── Owner: editar calendario de un instructor del tenant ─────────
 *
 * El owner también puede tocar horarios, fechas puntuales y pausas
 * de sus instructores sin tener que hacer login como ellos. Todas las
 * escrituras exigen que el `instructor_user_id` sea efectivamente un
 * instructor activo del tenant — chequeamos memberships.
 * ──────────────────────────────────────────────────────────────── */

async function assertInstructorOfTenant(instructorUserId: string, tenantId: string): Promise<boolean> {
  if (!instructorUserId) return false;
  const svc = getServiceClient();
  const { data } = await svc
    .from('memberships').select('id')
    .eq('tenant_id', tenantId).eq('user_id', instructorUserId)
    .eq('role', 'instructor').eq('status', 'active')
    .maybeSingle<{ id: string }>();
  return !!data;
}

export async function ownerAddAvailabilityAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  if (!(await assertInstructorOfTenant(instructorUserId, tenant.id))) return;
  const weekday = parseInt(String(formData.get('weekday') ?? ''), 10);
  const start = hhmmToMin(String(formData.get('start_time') ?? '').trim());
  const end = hhmmToMin(String(formData.get('end_time') ?? '').trim());
  const duration = parseInt(String(formData.get('slot_duration_min') ?? '60'), 10);
  ...
  await (svc.from('availability_rules') as any).insert({
    ...
    weekday, start_min: start, end_min: end,
    slot_duration_min: Math.min(480, Math.max(5, Number.isFinite(duration) ? duration : 60)),
    timezone: VALID_TZS.has(tz) ? tz : 'America/Argentina/Buenos_Aires'
  });
  revalidatePath(`/owner/equipo/${instructorUserId}/calendario`);
}

export async function ownerDeleteAvailabilityAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  const id = String(formData.get('id') ?? '');
  if (!id || !instructorUserId) return;
  const svc = getServiceClient();
  await svc.from('availability_rules').delete()
    .eq('id', id).eq('tenant_id', tenant.id)
...
export async function ownerAddCalendarDateAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  if (!(await assertInstructorOfTenant(instructorUserId, tenant.id))) return;
  const date = String(formData.get('date') ?? '').trim();
  ...
  await (svc.from('calendar_dates') as any).insert({
    tenant_id: tenant.id,
    instructor_user_id: instructorUserId,
    date, start_min: start, end_min: end,
    slot_duration_min: Math.min(480, Math.max(5, Number.isFinite(duration) ? duration : 60)),
    timezone: VALID_TZS.has(tz) ? tz : 'America/Argentina/Buenos_Aires',
    notes
  });
...
export async function ownerDeleteCalendarDateAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  const id = String(formData.get('id') ?? '');
  if (!id || !instructorUserId) return;
...
export async function ownerAddOverrideAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  if (!(await assertInstructorOfTenant(instructorUserId, tenant.id))) return;
  const startAt = String(formData.get('start_at') ?? '').trim();
...
export async function ownerDeleteOverrideAction(formData: FormData): Promise<void> {
  const { tenant } = await requireOwner();
  const instructorUserId = String(formData.get('instructor_user_id') ?? '');
  const id = String(formData.get('id') ?? '');
  if (!id || !instructorUserId) return;
