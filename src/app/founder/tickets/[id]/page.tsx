...
export default async function FounderTicketDetail({
  params
}: {
  ...
  const { data: ticket } = await svc
    ...
    .select("id, subject, status, created_at, tenant_id, tenants(name, slug)")
    .eq("id", id)
    .maybeSingle();
  if (!ticket) notFound();
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <p className="text-xs text-white/40 mt-1">
          {t.tenants?.name ?? '—'} · Estado: {t.status} · Abierto {new Date(t.created_at).toLocaleString('es-AR')}
        </p>
      </div>
            ...
            <li key={m.id} className={`rounded-xl border p-4 ${isFounder ? 'border-orange-500/30 bg-orange-500/5' : 'border-white/10 bg-white/[0.02]'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">
                  {isFounder ? '🛟 Equipo OfferNow' : (m.profiles?.display_name ?? m.profiles?.email ?? 'Owner')}
                </span>
      ...
      <ReplyForm ticketId={t.id} status={t.status} />
    </div>
  );
}
